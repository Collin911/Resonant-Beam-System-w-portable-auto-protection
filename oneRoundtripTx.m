function [U] = oneRoundtripTx(U_pre, H_fsdf,H_fsdf2, H_fsl, H_lens,H_lensShift,...
    H_fsf, B_lens, B_aper, B_lensshift, delta, H_shiftphase1, H_shiftphase2, tiltParam, ro, rp)
%注意：当存在倾斜角度时，为保证代码整体逻辑兼容性，默认传入的U_pre已经过斜面坐标变换
%本函数返回的光场是在接收端腔镜、经过斜面变换的光场

% 以接收端镜面为起始平面   
    U = Cal_AperturetoLens1(U_pre, H_fsl, B_lensshift, B_lensshift, delta);              %M2-L2入射表面
%     mesh(abs(U));
    U = Cal_lensImpact(U, H_lensShift, B_lensshift);                                 %L2入射表面-L2出射表面
    U = Cal_AperturetoLens2(U, H_fsdf2, B_aper, B_lensshift, delta, H_shiftphase2);             %L2出射表面-孔阑
%     mesh(abs(U));
...
%     mesh(abs(U));
    ...
    U = Cal_AperturetoLens1(U, H_fsf, B_aper, B_lens, delta);              %L1'出射表面-孔阑
        %mesh(abs(U_pre));
    U = Cal_AperturetoLens2(U, H_fsdf, B_lensshift, B_aper, delta, H_shiftphase1);         %孔阑到L2'入射表面
    if tiltParam ~= 0
        U = Cal_TransTiltPlane(U, B_lens, B_aper, psi, sampN, lambda, deltaF);
    end
%     mesh(abs(U));
    U = Cal_lensImpact(U, H_lensShift, B_lensshift);                                 %L2'入射表面-L2'出射表面
    U = Cal_AperturetoLens1(U, H_fsl, B_lensshift, B_lensshift, delta);              %L2'出射表面-M2'
