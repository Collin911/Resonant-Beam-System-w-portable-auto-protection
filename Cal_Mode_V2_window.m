function [S, finalDistribution, U_eigen, U_next, itr_times, Vp] = Cal_Mode_V2_window(H_fsw, windowTimes, H_fsl, H_lens,H_lensShift,...
...
%    finalDistribution =  (abs(U)./(max(max(abs(U))))).^2;                                              %输出幅值
        ...
        [U_next,Vp] = oneRoundtripWithPB_V3_window(U_eigen, H_fsw, windowTimes, H_fsl, H_lens,H_lensShift,...
       H_fsf, B_lens, B_aper, B_lensshift, delta, H_shiftwindow1, H_shiftwindow2, tiltParam, ro, rp, PB_path_len);  %这次像面即下次物面 
