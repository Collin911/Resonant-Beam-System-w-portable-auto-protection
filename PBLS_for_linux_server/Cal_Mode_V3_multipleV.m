function [S, U_eigen, U_next, itr_times, Vp, Vvector] = Cal_Mode_V3_multipleV(H_fsw, windowTimes, H_fsl, H_lens,H_lensShift,...
...
%    primaryU = InitialTiltWave(length, shift, lambda, r_CatEye);
   ...
   [firstU,~,~] = oneRoundtripWithPB_V4_multipleV(primaryU, H_fsw, windowTimes, H_fsl, H_lens,H_lensShift,...
...
%    firstU = gpuArray(firstU);
   s = gpuDevice();
   disp("GPU Running on " + s.Name)
   %开始迭代过程
   prevU = firstU;
   prevV = 1;
   itr = 0;
   while 1
        [U_eigen,~,~] = oneRoundtripWithPB_V4_multipleV(prevU, H_fsw, windowTimes, H_fsl, H_lens,H_lensShift,...
...
%    finalDistribution =  (abs(U)./(max(max(abs(U))))).^2;                                              %输出幅值
        ...
        [U_next,Vp,Vvector] = oneRoundtripWithPB_V4_multipleV(U_eigen, H_fsw, windowTimes, H_fsl, H_lens,H_lensShift,...
       H_fsf, B_lens, B_aper, B_lensshift, delta, H_shiftwindow1, H_shiftwindow2, tiltParam, ro, rp, PB_path_len);  %这次像面即下次物面 
