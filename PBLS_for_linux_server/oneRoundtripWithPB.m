function [U] = oneRoundtripWithPB(U_pre, H_fsdf,H_fsdf2, H_fsl, H_lens,H_lensShift,...
    ...
    if(~exist('PBmodulation','var'))
        PBmodulation_len = 0;  % 如果未出现该变量，则对其进行赋值
    end
% 以孔阑为起始平面   
    %mesh(abs(U_pre));
...
%     mesh(abs(U));
    ...
    if ro ~= 1 && rp~= 0
        ...
        H_fsf, B_lens, B_aper, B_lensshift, delta, H_shiftphase1, H_shiftphase2, tiltParam); %计算保护光一次往返
    end
    [~, ~, ~, ~, ~, ~, lambda] = Para_FFTAlgorithm(0);
    pU = Modulate(pU, lambda, PBmodulation_len);
    U_M2_total = U .* ro + pU .* to .* rp;
    
    U = Cal_AperturetoLens1(U_M2_total, H_fsl, B_lensshift, B_lensshift, delta);              %M2-L2'入射表面
%     mesh(abs(U));
