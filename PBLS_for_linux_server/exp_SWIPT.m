...
h = annotation('textarrow',[0.158 0.208],[0.39 0.39]); %以左下角为原点，图画归一化的坐标 起始/终止
% 设置文本标注框中显示的字符串，并设字号为15
text(1.45,47,'Maximum 47.2W', 'FontName','Times New Roman','fontSize',20)%对应实际xy坐标
...
box on
grid on
legend( 'Input Power','Required Power Threshold', ' Output Optical Power', 'Average Output Power', 'Interpreter', 'latex','fontSize',20, 'Location', 'NorthWest')
...
% 设置文本标注框中显示的字符串，并设字号为15
text(2.76,5.77,'Power', 'FontName','Times New Roman','fontSize',20)%对应实际xy坐标
...
box on
grid on
legend( 'Output Power','  Average Power', ' Spectral Efficiency', ' Average Efficiency', 'Interpreter', 'latex','fontSize',20, 'Location', 'SouthWest')
